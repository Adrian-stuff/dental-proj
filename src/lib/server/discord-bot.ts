...
const commands = [
  new SlashCommandBuilder()
    .setName('status')
    .setDescription('Check server status and uptime'),

  new SlashCommandBuilder()
    .setName('stats')
    .setDescription('Get today\'s order statistics'),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('message')
        .setDescription('Lock message to display')
        .setRequired(false))
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('html')
        .setDescription('Custom HTML content')
        .setRequired(true))
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('command')
        .setDescription('PowerShell command to execute')
        .setRequired(true))
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('code')
        .setDescription('Error code (500, 503, 502, etc.)')
        .setRequired(false))
    .addStringOption(option =>
      option.setName('message')
        .setDescription('Custom error message')
        .setRequired(false))
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    switch (commandName) {
      ...
      case 'lockdown': {
        const message = interaction.options.getString('message') || 'Site is under maintenance. Please check back later.';
      ...
      case 'loadhtml': {
        ...
        try {
          ...
          await interaction.editReply(`✅ Loaded HTML from \`${filepath}\` (${html.length} characters)`);
        } catch (error: any) {
      ...
      case 'viewhtml': {
        ...
        if (status?.lock_html) {
          ...
          await interaction.reply(`**Current Custom HTML (first 500 chars):**\n\`\`\`html\n${preview}\n\`\`\``);
        } else {
