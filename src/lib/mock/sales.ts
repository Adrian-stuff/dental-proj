...
export function getMockRecords({ status, exactDate, startDate, endDate, remarks }: { status?: string | null; exactDate?: string | null; startDate?: string | null; endDate?: string | null; remarks?: string | null; }) {
...
export function getMockSupplies({ exactDate, startDate, endDate }: { exactDate?: string | null; startDate?: string | null; endDate?: string | null; }) {
  let rows = MOCK_SUPPLIES.slice();
  if (exactDate) rows = rows.filter((s) => s.supplyDate === exactDate);
  if (startDate && endDate) rows = rows.filter((s) => s.supplyDate >= startDate && s.supplyDate <= endDate);
  return rows;
}
