...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const summary: Summary = {
    ...
    totalAmount: ordersWithItems.reduce(
      (sum, order) => sum + Number(order.orderTotal || 0),
      0
    ),
    paidAmount: ordersWithItems.reduce(
    ...
    caseTypeTotals: ordersWithItems.reduce((acc, order) => {  // Add this calculation
      if (order.items) {
        const caseType = allCaseTypes.find(ct => ct.caseTypeId === order.items.caseTypeId);
        if (caseType) {
          acc[caseType.caseTypeName] = (acc[caseType.caseTypeName] || 0) +
  ...
  const chartData = ordersWithItems.reduce((acc, order) => {
  ...
  const clinicChartData = ordersWithItems.reduce((acc, order) => {
  ...
  const sortedClinicChartData = Object.fromEntries(
    Object.entries(clinicChartData)
      .sort(([, a], [, b]) => b - a)
  );
