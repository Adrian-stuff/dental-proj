...
import { caseTypes, clinics, doctors, history, orderItems, orders, records } from '$lib/server/db/schema';
  ...
  default: async ({ cookies, request }) => {
    ...
    try {
      ...
      recordId = await db.transaction(async (tx) => {
        ...
        const orderID = await tx.insert(orders).values({
        ...
        } as typeof orders.$inferInsert).returning({ id: orders.orderId });
        ...
        const record = await tx.insert(records).values({
          ...
          remarks: "pending",
        } as typeof records.$inferInsert).returning({ id: records.recordId })
        return record
      }).then(async (record) => {
